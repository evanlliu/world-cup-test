...
- 以 v1 为基础实现 v102 的顶部比分卡片紧凑效果。
- PC 端和移动端同步缩小预测页顶部比分卡片高度。
- 缩小队旗、比分、状态文字、前瞻 / 直播按钮间距，让下方内容首屏显示更多。
- Service Worker 缓存名更新为 `wc2026-pwa-v2-compact-hero`。

# 2026 世界杯赛程 PWA

这是一个适合部署到 GitHub Pages 的 2026 世界杯赛程 PWA 页面，支持 PC 和手机端访问，也适合在 iPhone Safari 中“添加到主屏幕”当作 App 使用。

当前版本：v2 顶部比分卡片紧凑版（PC / 移动端）  
基础优化来源：v94 刷新按钮图标版
